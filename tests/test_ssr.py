...
def test_load_ssr_c():
    ...
    year = 2017
    ssr_c = ssr._load_ssr_c(path2flat, year, spark)
    assert ssr_c.columns == ['NUM_ENQ', 'ETA_NUM', 'RHA_NUM', 'EXE_SOI_DTD', 'EXE_SOI_DTF']
    

def test_load_ssr_stc():
    path2csv_ssr = "data/raw/SSR_"
    year = 2017
    ssr_stc = ssr._load_ssr_stc(path2csv_ssr, year, spark)
    assert ssr_stc.columns == ["ETA_NUM", "RHA_NUM", "SEJ_FAC_AM", "TOT_MNT_AM", "TOT_MNT_AMC", "REM_BAS_MNT",
...
def test_agg_ssr():
    ...
    year = 2017
    filter_etab = ['600100093', '600100101'] # fake, not comprehensive list
    list_output_var = ["NUM_ENQ", "poste_ag", "poste", "nb_sejour", "NBSEJ_CPLT_COURT", "NBSEJ_CPLT_LONG",
    ...
    rac_ssr = ssr.agg_ssr(path2flat, path2csv_ssr, path2_ir_orc, year, filter_etab, list_output_var, spark)
...
def test_agg_med():
    ...
    list_output_var = ["NUM_ENQ", "poste_ag", "poste", "nb_sejour", "NBSEJ_CPLT_COURT", "NBSEJ_CPLT_LONG",
                       ...
                       "rac",  "rac_amo_sup", "MNT_TM", "MNT_PF", "MNT_FJ", "dep_lien_ald", "rac_lien_ald"]
    rac_ssr = ssr.agg_ssr(path2flat, path2csv_ssr, path2_ir_orc, year, filter_etab, list_output_var, spark)
    rac_ssr = (rac_ssr.withColumn("BDI_DEP", psf.lit("01"))
           .withColumn("BDI_COD", psf.lit("456"))
           .withColumn("COD_SEX", psf.lit("1"))
           .withColumn("AGE_ANN", psf.lit(15))) # par ce que comme ce sont des fausses données, la jointure peut
    # n'ajouter aucune colonne dans agg_ssr si aucune clef commune
    ssr_public_sus = ssr.agg_med(path2flat, path2csv_ssr, rac_ssr, year, list_output_var, spark)
...
def test_agg_ace():
    ...
    list_output_var = ["NUM_ENQ", "poste_ag", "poste", "nb_sejour", "NBSEJ_CPLT_COURT", "NBSEJ_CPLT_LONG",
                       "NBSEJ_PARTL_COURT", "NBSEJ_PARTL_LONG", "NBJ_CPLT", "NBJ_PARTL", "dep_tot", "remb_am",
                       "rac",  "rac_amo_sup", "MNT_TM", "MNT_PF", "MNT_FJ", "dep_lien_ald", "rac_lien_ald"]
    rac_ace = ssr.agg_ace(path2flat, path2csv_ssr, path2nomenclature, path2_ir_orc, filter_etab, year, spark, list_output_var)
